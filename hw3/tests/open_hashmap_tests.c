...
static int traverse_good_cb(OpenHashmapNode *node) {
    LOG_DEBUG("KEY: %s", bdata((bstring)node->key));
    traverse_called++;
    return 0;
}

static int traverse_fail_cb(OpenHashmapNode *node) {
    LOG_DEBUG("KEY: %s", bdata((bstring)node->key));
    traverse_called++;
...
char *test_simple() {
    ...
    rc = OpenHashmap_traverse(map, traverse_good_cb);
    MU_ASSERT(rc == 0, "Failed to traverse.");
    MU_ASSERT(traverse_called == 3, "Wrong count traverse.");

    traverse_called = 0;
    rc = OpenHashmap_traverse(map, traverse_fail_cb);
