...
OpenHashmap *_OpenHashmap_create(OpenHashmap_compare compare,
    ...
    CHECK(DArray_max(ret->buckets) == ret->number_of_buckets,
          "Couldn't initialize buckets.");
    CHECK(DArray_len(ret->buckets) == ret->number_of_buckets,
          "Couldn't initialize buckets.");
...
int OpenHashmap_set(OpenHashmap *map, void *key, void *data) {
    ...
    do {
        node = DArray_get(map->buckets, i);
        if (node == NULL) {
            node = calloc(1, sizeof(OpenHashmapNode));
            CHECK_MEM(node);
            DArray_set(map->buckets, i, node);
        }
        if (IS_VACANT(node)) {
            node->data = data;
            node->hash = hash;
